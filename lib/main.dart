...
class CustomProductDialog extends StatefulWidget{const CustomProductDialog({super.key});@override State<CustomProductDialog> createState()=>_CustomProductDialogState();}
class _CustomProductDialogState extends State<CustomProductDialog>{
  ...
  @override Widget build(BuildContext x)=>AlertDialog(title:const Text('Мій продукт'),content:SingleChildScrollView(child:Column(mainAxisSize:MainAxisSize.min,children:[
    ...
    const SizedBox(height:8),
    const Align(alignment:Alignment.centerLeft,child:Text('Дані для швидкого введення',style:TextStyle(fontWeight:FontWeight.w600))),
  ...
  ])),actions:[TextButton(onPressed:()=>Navigator.pop(x),child:const Text('Скасувати')),FilledButton(onPressed:n.text.trim().isNotEmpty&&c.text.isNotEmpty?()=>Navigator.pop(x,Product(
    id:'custom_${DateTime.now().microsecondsSinceEpoch}',name:n.text.trim(),category:cat.text.trim(),carbs:_num(c)??0,
